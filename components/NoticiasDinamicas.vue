<template>
  <div class="flex flex-col md:flex-row gap-[3vw] md:gap-[1vw]">
    ...
    <!-- ============================================ -->
    <div class="md:w-[70%]">
      <!-- Skeleton: solo si NO hay datos Y está cargando -->
      <div v-if="showSkeleton" class="w-full">
      ...
      <div
        ...
        @click="irANoticia(noticiaPrincipal)"
      >
        <div class="w-full">
      ...
      <div v-else class="w-full text-center py-12 bg-gray-50 rounded-lg">
        ...
        <button
          @click="recargar"
          class="mt-4 px-4 py-2 bg-[#E03636] text-white rounded-lg hover:bg-[#C12F2F] transition text-[3vw] md:text-[1vw]"
        >
          Reintentar
        </button>
      </div>
    ...
    <div class="md:w-[30%] flex flex-col gap-[3vw] md:gap-[1vw]">
      ...
      <div v-else-if="!showSkeleton" class="text-center py-8 bg-gray-50 rounded-lg">
...
const { noticiasImportantes, loading, hasData } = storeToRefs(noticiasStore)

// 🔥 Mostrar skeleton SOLO si:
// 1. Está cargando
// 2. Y NO hay datos previos guardados
const showSkeleton = computed(() => {
  return loading.value && !hasData.value
})

// ==========================================
// HELPERS
// ==========================================
const tituloResaltado = (texto) => {
  if (!texto) return ''
...
const obtenerImagen = (noticia) => {
  ...
  return noticia.featuredImage?.url || noticia.imagen || '/images/default-news.jpg'
}
...
const recargar = async () => {
  await noticiasStore.ensureLoaded(true)
}
