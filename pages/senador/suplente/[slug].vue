<template>
  <div class="py-[6vw] sm:py-[3vw]">
    <div v-if="suplente" class="w-[92%] sm:w-[85%] mx-auto">
      <!-- Botón volver -->
      <button 
        @click="volver"
        class="inline-flex items-center text-senado-primary hover:underline mb-[3vw] sm:mb-[1.5vw] text-[3.3vw] sm:text-[1.1vw] cursor-pointer"
      >
        ...
        <div class="bg-[#EDEEED]">
          <div class="grid grid-cols-1 sm:grid-cols-12 border-b border-gray-200 w-[92%] sm:w-[85%] mx-auto gap-[2vw] sm:gap-0">
            ...
            <div class="sm:col-span-3 p-[3vw] sm:p-[1.2vw] flex flex-col items-center justify-center">
              ...
              >
                <img 
                  :src="suplente.foto || '/images/default-avatar.png'" 
                  :alt="suplente.name"
            ...
            <div class="sm:col-span-6 py-[3vw] sm:py-[1.2vw] sm:ml-[-1vw] flex flex-col justify-center text-center sm:text-left">
              <h1 class="text-[5.4vw] sm:text-[1.8vw] font-bold text-senado-primary">{{ suplente.name }}</h1>
              <p class="text-[3.3vw] sm:text-[1.1vw] text-black">Senador Suplente por {{ suplente.department }}</p>
              <p class="text-[2.7vw] sm:text-[0.9vw] text-gray-500">
                Senador Titular: 
                <NuxtLink 
                  :to="`/senador/${suplente.titularSlug}`" 
                  class="text-senado-primary hover:underline font-medium text-[2.7vw] sm:text-[0.9vw]"
                >
                  {{ suplente.titularNombre }}
                </NuxtLink>
              </p>
              <p v-if="suplente.cargo && suplente.cargo !== 'No disponible'" class="font-semibold text-[3vw] sm:text-[1vw] leading-tight">
                {{ suplente.cargo }}
              </p>
            </div>
            ...
            <div class="space-y-[3vw] sm:space-y-[1vw] text-[2.7vw] sm:text-[0.9vw]">
              ...
              <div v-if="suplente.facebook || suplente.twitter || suplente.instagram || suplente.youtube || suplente.tiktok" class="grid grid-cols-[40%_60%]">
                ...
                <div class="flex gap-[1.5vw] sm:gap-[0.5vw] flex-wrap">
                  ...
                  <a v-if="suplente.twitter" :href="suplente.twitter" target="_blank" rel="noopener noreferrer" class="w-[3.5em] sm:w-[2.2em] h-[3.5em] sm:h-[2.2em] rounded-full bg-black flex items-center justify-center hover:bg-gray-800 transition-colors">
                  ...
                  <a v-if="suplente.instagram" :href="suplente.instagram" target="_blank" rel="noopener noreferrer" class="w-[3.5em] sm:w-[2.2em] h-[3.5em] sm:h-[2.2em] rounded-full bg-black flex items-center justify-center hover:bg-gray-800 transition-colors">
                  ...
                  <a v-if="suplente.youtube" :href="suplente.youtube" target="_blank" rel="noopener noreferrer" class="w-[3.5em] sm:w-[2.2em] h-[3.5em] sm:h-[2.2em] rounded-full bg-black flex items-center justify-center hover:bg-gray-800 transition-colors">
                  ...
                  <a v-if="suplente.tiktok" :href="suplente.tiktok" target="_blank" rel="noopener noreferrer" class="w-[3.5em] sm:w-[2.2em] h-[3.5em] sm:h-[2.2em] rounded-full bg-black flex items-center justify-center hover:bg-gray-800 transition-colors">
        ...
        <div class="bg-[#EDEEED] border-b border-gray-200 px-[3vw] sm:px-[1.5vw] py-[2vw] sm:py-[1vw]">
          <div class="flex flex-wrap items-center justify-between gap-[2vw] sm:gap-[1vw]">
            ...
            <div class="flex items-center gap-[2vw] sm:gap-[1vw]">
              <span v-if="suplente.proyectosLey.cantidad > 3" 
                ...
                {{ suplente.proyectosLey.cantidad }} proyectos presentados
              </span>
              <span v-else 
                    class="text-[2vw] sm:text-[0.7vw] font-medium text-senado-primary bg-senado-primary/10 px-[2vw] sm:px-[1vw] py-[0.3vw] sm:py-[0.2vw] rounded-full">
                {{ suplente.proyectosLey.cantidad }} proyecto{{ suplente.proyectosLey.cantidad > 1 ? 's' : '' }}
              </span>
            </div>
          </div>
        </div>
          ...
          <div v-if="suplente.proyectosLey.lista.length > 3" 
            ...
            <div class="flex items-center gap-[2vw] sm:gap-[0.8vw] text-[2.4vw] sm:text-[0.8vw] text-gray-500">
              ...
              <span>
                ...
                <strong> {{ suplente.proyectosLey.cantidad }}</strong> proyectos presentados por el senador suplente.
              </span>
            </div>
          </div>
        </div>
    ...
    <div v-else class="text-center py-[10vw] sm:py-[5vw]">
      <h2 class="text-[5vw] sm:text-[2vw] font-bold text-gray-600">Suplente no encontrado</h2>
        ...
        >
          <div class="relative max-w-[90vw] sm:max-w-[80vw] max-h-[85vh] flex items-center justify-center" @click.stop>
            ...
            <div class="absolute bottom-[-8vw] sm:bottom-[-3vw] left-1/2 -translate-x-1/2 bg-black/70 text-white text-[2.4vw] sm:text-[0.9vw] px-[4vw] sm:px-[2vw] py-[1.5vw] sm:py-[0.8vw] rounded-[1.5vw] sm:rounded-[0.6vw] text-center backdrop-blur-sm whitespace-nowrap max-w-[80vw] sm:max-w-none">
              <span class="font-bold">{{ suplente?.name }}</span>
              <span class="mx-[1.5vw] sm:mx-[0.8vw] text-gray-400">|</span>
              <span>Senador Suplente por {{ suplente?.department }}</span>
            </div>
          </div>
        </div>
      </transition>
...
const getAsientoHemiciclo = (seatNumber) => {
  const asientos = {
    ...
    6: '/hemiciclo/37NiltonCondori.svg',
    ...
    12: '/hemiciclo/38LeonardoRoca.svg',
    13: '/hemiciclo/39LorgioFernando.svg',
    ...
    24: '/hemiciclo/40CarolinaGiese.svg',
    ...
    32: '/hemiciclo/41LuzmayaZelaya.svg',
    33: '/hemiciclo/42RamiroMamani.svg',
