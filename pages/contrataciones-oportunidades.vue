          ...
          <table class="management-table">
            <thead>
              <tr>
                ...
                <th @click="ordenarPor('gestion')" class="sortable">
                  Gestión
                  <span class="sort-icon">{{ getSortIcon('gestion') }}</span>
                </th>
            ...
            <tbody>
              <tr v-for="doc in documentosFiltrados" :key="doc.id" class="management-row">
                ...
                <td>
                  <span class="tipo-badge" :class="getTipoClase(doc.tipo)">
                    {{ doc.tipo }}
                  </span>
                </td>
                <td>{{ doc.gestion || '-' }}</td>
...
const API_URL = 'https://api.senado.gob.bo/contrataciones-oportunidades'
...
const getTipoClase = (tipo) => {
  const tipos = {
    ...
    'Resultados': 'tipo-resultados',
    'Informe': 'tipo-informe',
    'Programa Anual': 'tipo-pac'
  }
  return tipos[tipo] || 'tipo-otro'
}
...
const filtrarDocumentos = () => {
  ...
  documentosFiltrados.value = documentos.value.filter(doc => 
    ...
    (doc.tipo && doc.tipo.toLowerCase().includes(termino)) ||
...
const cargarDocumentos = async () => {
  ...
  try {
    ...
    //tipo: PAC, Convocatoria, Resultados, Informe, Programa Anual
    
    documentos.value = [
      {
        id: 1,
        nombre: 'Programa Anual de Contrataciones 2026',
        descripcion: 'PAC al 25.08.2026',
        tipo: 'Programa Anual',
        gestion: '2026',
        fecha: '2026-08-25T10:30:00',
        url: '/pdfs/Recursos-humanos/pac-2026.pdf'
      }
    ]
